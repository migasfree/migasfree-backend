...
@override_settings(MIGASFREE_TMP_DIR='/tmp')
class TestIsSafeFilename(TestCase):
    ...
    def test_safe_filename_simple(self):
        """Simple filename inside tmp dir should be safe."""
    ...
    def test_safe_filename_outside_tmp(self):
        """Path outside TMP dir should be unsafe."""
        self.assertFalse(is_safe_filename('/home/user/documents/file.pdf'))

    def test_safe_filename_with_subdirectory(self):
        """Path with subdirectory inside tmp should be safe."""
        self.assertTrue(is_safe_filename('/tmp/documents/file.pdf'))

    def test_unsafe_filename_with_parent_directory(self):
        """Filename with parent directory traversal (..) should be unsafe."""
        self.assertFalse(is_safe_filename('/tmp/../etc/passwd'))

    def test_safe_filename_with_embedded_parent(self):
        """Filename with embedded parent traversal inside tmp should be safe (normalized)."""
        self.assertTrue(is_safe_filename('/tmp/user/../etc/shadow'))

    def test_unsafe_filename_with_backslash(self):
        """Filename with backslash should be unsafe."""
        self.assertFalse(is_safe_filename('/tmp/file\\name.txt'))

    def test_unsafe_filename_with_pipe(self):
        """Filename with pipe character should be unsafe."""
        self.assertFalse(is_safe_filename('/tmp/file|command'))

    def test_unsafe_filename_with_semicolon(self):
        """Filename with semicolon should be unsafe."""
        self.assertFalse(is_safe_filename('/tmp/file;rm -rf /'))
    ...
    def test_unsafe_filename_with_dollar(self):
        ...
        self.assertFalse(is_safe_filename('/tmp/$variable'))

    def test_unsafe_filename_with_redirect(self):
        """Filename with redirect characters should be unsafe."""
        self.assertFalse(is_safe_filename('/tmp/file>output'))
        self.assertFalse(is_safe_filename('/tmp/file<input'))
    ...
    def test_safe_filename_empty(self):
        """Empty filename should be unsafe (not absolute)."""
...
class TestSecureFunctions(TestCase):
    ...
    def test_sign_command_structure(self, mock_path_join, mock_run):
        ...
        # If not called, sign returned False (unsafe filename)
        self.assertTrue(mock_run.called, 'subprocess.run not called, likely unsafe filename')
